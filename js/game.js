...
function save_current_blocks_to_playfield() {
  for (var cany = 0; cany < current_block.size; cany++) {
    for (var canx = 0; canx < current_block.size; canx++) {
      if (
        cany + current_block.positiony >= playfield.height ||
        canx + current_block.positionx >= playfield.width
      ) {
        continue;
      }

      playfield.field[cany + current_block.positiony][
        canx + current_block.positionx
      ] += current_block.field[cany][canx];
    }
  }
}
...
function generate_block(block_type, temp_position_y, temp_position_x) {
  ...
  return new Block(
    ...
    block_type,
    true,
    get_field(block_type, field_size)
  );
}
