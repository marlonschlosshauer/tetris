...
Toolkit.prototype.draw_block_full = function(y, x) {
	...
	this.brush.fillRect(
		x * (BLOCK_BORDER + BLOCK_SIZE + BLOCK_BORDER) +
		BLOCK_BORDER * 2,
		y * (BLOCK_BORDER + BLOCK_SIZE + BLOCK_BORDER) +
		BLOCK_BORDER * 2,
...
Toolkit.prototype.print_currentblock_to_canvas = function (temp_block)
{
	for(var y = 0 ; y < temp_block.size; y++)
	{
		for(var x = 0 ; x < temp_block.size; x++)
		{
			...
			{
				this.draw_block_full(y + temp_block.positiony - INITIALY,x + temp_block.positionx);
			}
		}
	}
};
...
Toolkit.prototype.print_ghost_to_canvas = function (temp_playfield, temp_block)
{
	...
	for(var y = 0 ; y < temp_block.size; y++)
	{
		for(var x = 0 ; x < temp_block.size; x++)
		{
			...
			{
				this.draw_block_ghost(y - INITIALY + temp_block.positiony,x + temp_block.positionx);
			}
		}
	}
};
